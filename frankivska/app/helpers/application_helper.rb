module ApplicationHelper
  ...
  def description(t = SEO[:description])
    content_for :seo_description do
      t
    end
  end
  
  def keywords(t = SEO[:keywords])
  ...
  def flag_link(locale)
    link_to locale, url_for(request.parameters.merge({:locale => locale})), :title => locale, :id => locale
  end
